...
export default function ProductsPage() {
  ...
  return (
    <div className="pt-16 min-h-screen">
      {/* Page header */}
      <div className="px-8 py-12 border-b border-flex-black/10">
        <div className="max-w-[1400px] mx-auto">
          ...
          <motion.p
            ...
            className="font-mono text-[10px] uppercase tracking-[0.4em] mt-3 text-flex-black/40"
          >
      ...
      {/* Filter bar */}
      <div className="sticky top-16 z-30 bg-flex-white border-b border-flex-black/10 shadow-sm">
          <div className="max-w-[1400px] mx-auto px-8 py-3 flex flex-wrap gap-2 items-center">
          <Filter size={12} className="text-flex-black/30 mr-1" />
          {categories.map((cat) => (
            <button
              key={cat}
          ...
          <div className="w-px h-4 bg-flex-black/15 mx-1" />
          {brands.map((b) => (
            <button
              key={b.slug}
          ...
          <div className="ml-auto flex gap-2">
            <button onClick={() => setView("grid")} className={`p-2 rounded-sm transition-colors ${view === "grid" ? "bg-flex-black text-flex-yellow" : "text-flex-black/40 hover:text-flex-black"}`}>
              <Grid size={14} />
            </button>
            <button onClick={() => setView("list")} className={`p-2 rounded-sm transition-colors ${view === "list" ? "bg-flex-black text-flex-yellow" : "text-flex-black/40 hover:text-flex-black"}`}>
              <LayoutList size={14} />
            </button>
            <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-flex-black/30 flex items-center ml-2">
      ...
      <div className="max-w-[1400px] mx-auto px-8 py-10">
        <motion.div
          ...
          className={view === "grid"
            ...
            : "flex flex-col divide-y divide-flex-black/10"
          }
        >
                  ...
                  <Link href={`/products/${product.slug}`} className="group block">
                    ...
                    <div>
                      ...
                      <div className="flex justify-between items-start gap-2">
                        <p className="font-display font-bold text-[13px] uppercase leading-tight">{product.name}</p>
                        <p className="font-display font-black text-[13px] flex-shrink-0">{formatPrice(product.price)}</p>
                      </div>
                    </div>
                  </Link>
                ) : (
                  <Link href={`/products/${product.slug}`} className="group flex items-center gap-5 py-4 hover:bg-flex-yellow-bright/20 px-3 rounded-sm transition-colors duration-200">
                    <div className="relative w-16 h-20 flex-shrink-0 overflow-hidden rounded-sm bg-gray-100">
                      <Image src={product.image} alt={product.name} fill className="object-cover" sizes="64px" />
                    </div>
                    <div className="flex-1">
                      <p className="font-mono text-[9px] uppercase tracking-[0.3em] text-flex-black/40">{product.brand}</p>
                    ...
                    <p className="font-display font-black text-lg">{formatPrice(product.price)}</p>
                  </Link>
                )}
              </motion.div>
            ))}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20">
            <p className="font-display font-black text-4xl uppercase">No items found</p>
