...
export default function FlexHomePage() {
  ...
  return (
    <>
      {/* HERO */}
      <section className="relative min-h-screen pt-16 bg-flex-black text-flex-white overflow-hidden flex items-center">
        <AnimatePresence>
          {scanActive && (
            <motion.div
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-flex-black via-flex-black/85 to-flex-black/30" />
        </div>
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-8 w-full py-24">
          <div className="max-w-3xl">
            ...
            <div className="overflow-hidden mb-1">
              <motion.h1 initial={{ y: "110%" }} animate={{ y: 0 }} transition={{ duration: 0.75, delay: 0.35, ease: [0.16, 1, 0.3, 1] }} className="font-display font-black uppercase text-flex-white" style={{ fontSize: "clamp(3.5rem,10vw,9rem)", lineHeight: 0.92 }}>
                GET
              </motion.h1>
            </div>
            ...
            <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }} className="font-mono text-[11px] text-flex-white/50 tracking-wide mb-8 max-w-sm">
              LV · Prada · Goyard · Nike — real pieces, real prices, shipped to your door.
            </motion.p>

            <div className="flex flex-wrap gap-3">
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.85 }}>
                <Link href="/products" className="inline-flex items-center gap-2.5 bg-flex-yellow text-flex-black font-mono font-bold text-[11px] uppercase tracking-[0.3em] px-7 py-3.5 hover:bg-white transition-colors duration-200">
                  Shop the Drop <ArrowRight size={13} />
                </Link>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.95 }}>
                <Link href="/about" className="inline-flex items-center gap-2.5 border border-flex-white/30 text-flex-white font-mono font-bold text-[11px] uppercase tracking-[0.3em] px-7 py-3.5 hover:border-flex-white hover:bg-flex-white/10 transition-all duration-200">
                  Our Story
                </Link>
              </motion.div>
            </div>
      ...
      <section className="py-16 px-8 max-w-[1400px] mx-auto">
        <div className="flex items-end justify-between mb-10">
          ...
          <Link href="/products" className="hidden md:flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.3em] text-flex-black/50 hover:text-flex-black transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="md:col-span-2">
            <Link href={`/products/${featuredProducts[0]?.slug}`} className="group block relative aspect-[16/10] overflow-hidden rounded-sm bg-gray-100">
          ...
          <div className="flex flex-col gap-4">
            {featuredProducts.slice(1, 3).map((product) => (
              <Link key={product.id} href={`/products/${product.slug}`} className="group relative flex-1 min-h-[180px] overflow-hidden rounded-sm bg-gray-100 block">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {featuredProducts.slice(3, 7).map((product) => (
            <Link key={product.id} href={`/products/${product.slug}`} className="group relative aspect-[3/4] overflow-hidden rounded-sm bg-gray-100 block">
      ...
      <section className="py-16 px-8 max-w-[1400px] mx-auto">
        <div className="flex items-end justify-between mb-10">
          ...
          <Link href="/products" className="font-mono text-[10px] uppercase tracking-[0.3em] text-flex-black/50 hover:text-flex-black flex items-center gap-1.5 transition-colors">
      ...
      <section className="relative overflow-hidden mx-8 mb-16 rounded-sm">
        <div className="relative h-[42vh] flex items-center">
          ...
          <div className="absolute inset-0 bg-flex-black/65" />
          <div className="relative z-10 px-10 w-full">
            ...
            <Link href="/products" className="inline-flex items-center gap-2.5 bg-flex-yellow text-flex-black font-mono font-bold text-[11px] uppercase tracking-[0.3em] px-7 py-3.5 hover:bg-white transition-colors">
      ...
      {/* SOCIAL */}
      <section className="py-12 px-8 bg-flex-black">
        <div className="max-w-[1400px] mx-auto grid grid-cols-3 gap-4">
          {[
            { icon: "📱", platform: "TikTok", desc: "Drops, fits & unboxings" },
      ...
      {/* FOOTER */}
      <footer className="bg-flex-black text-flex-white py-10 px-8 border-t border-flex-yellow/30">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex gap-3">
            {["TikTok", "Instagram", "WhatsApp"].map((s) => (
              <a key={s} href="#" className="font-mono text-[9px] uppercase tracking-[0.3em] text-flex-white/40 hover:text-flex-yellow transition-colors border border-flex-white/10 hover:border-flex-yellow px-3 py-2">{s}</a>
            ))}
          </div>
          <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-flex-white/20">© 2025 FlexHaus SA</p>
        </div>
      </footer>
    </>
  );
